## RSVB G epicc visulize
library(reshape2)
## read the raw dataframe
raw1<-read.table("EpiCC/RSV_EpiCC_Data/RSVB_G_classI_epicc_share_raw.csv", 
                 sep=",", header=TRUE,check.names=FALSE, stringsAsFacto=F, fill=TRUE,row.names = 1)
raw2<- read.table("EpiCC/RSV_EpiCC_Data/RSVB_G_classII_epicc_share_raw.csv", 
                  sep=",", header=TRUE,check.names=FALSE, stringsAsFacto=F, fill=TRUE,row.names = 1)
...
write.csv(vac, "EpiCC/RSVB_G/RSVB_G_vac.csv")
## after code the year and region

vac_meta<-read.csv("EpiCC/RSVB_G/RSVB_G_vac_code2.csv")
...
RSVB_G_mds<-ggplot(mds2, aes(x=Dim.1, y=Dim.2, color=groups)) + 
  geom_point()+
...
t1 <- read.tree(file = "EpiCC/RSVB_G/RSVB_G_subsample_epicc.nwk")
p1<-ggtree(t1) +
...
taxa<-read.csv("EpiCC/RSVB_G/RSVB_G_epicc_subsample_taxa.csv")
...
crossT_dist<-crossT_dis%>%
  select(`KU316116-B-1977-USA-7_16_77-GB3`)
...
RSVB_G_immu<-ggplot(crossT_dist, aes(x=as.numeric(year), y=`KU316116-B-1977-USA-7_16_77-GB3`,color=groups)) + 
  geom_point()+
...
ham_genetic<- read.table("EpiCC/RSVB_G/RSVB_G_epicc_subsample_genetic_hamming.csv", 
                         sep=",", header=TRUE,check.names=FALSE, stringsAsFacto=F, fill=TRUE,row.names = 1)

ham_dist<-ham_genetic%>%
  select(`KU316116-B-1977-USA-7/16/77-GB3`)
...
ham_dist<-left_join(ham_dist,mds2,by=c("Accession"="Accession","Subtype"="Subtype",
                                       "year"="year"))
RSVB_G_ham<-ggplot(ham_dist, aes(x=as.numeric(year), y=`KU316116-B-1977-USA-7/16/77-GB3`,color=groups)) + 
  geom_point()+
  scale_colour_brewer(palette = "Set2",name="T-cell immuno-clusters")+
...
ggarrange(RSVA_G_mds,RSVA_G_ML,RSVA_G_immu,RSVA_G_ham,
          RSVB_G_mds,RSVB_G_ML,RSVB_G_immu,RSVB_G_ham,
